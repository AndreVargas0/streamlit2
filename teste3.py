...
if df is not None and df_stations is not None:
    ...
    st.sidebar.subheader("Previsão de bicicletas disponíveis")
    ...
    input_data = pd.DataFrame({
        'user_gender': [0],  # Assumindo '0' para feminino
    ...
    previsao_bicicletas = int(predicted_bikes[0])
    ...
    plt.title('Gráfico de Dispersão - Previsões vs Reais')
    st.pyplot(plt)

    # R²
    st.write(f"Coeficiente de Determinação (R²): {r2:.4f}")

    # Gráfico de Erro Quadrático Médio
    plt.figure(figsize=(8, 6))
    plt.bar(['MSE', 'RMSE'], [mse, rmse], color=['green', 'orange'])
    plt.title('Métricas de Erro - MSE e RMSE')
    plt.ylabel('Erro')
    st.pyplot(plt)
